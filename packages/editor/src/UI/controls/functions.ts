 ...
 * whatever was drawing it - `CreateIcon` above throws by design for a name FD
 * does not have (see `need()` in `core/need.ts`), and a name can come straight
 * out of a blueprint. Falls back
 ...
 *
 * Takes a builder rather than a name, so it covers whatever builds the icon,
 * not only `CreateIcon`. Lives here, beside the builders it guards, rather
