...
import { Book, countNestedBlueprints } from '../core/Book'
...
function tryCreateIcon(name: string | undefined, size: number): Container | undefined {
    ...
    return F.SafeIcon(name, () => F.CreateIcon(name, size))
}
...
function buildRows(
    ...
    for (const entry of entries) {
        ...
        if (entry.blueprint) {
            ...
            if (depth >= MAX_DEPTH) {
                flatIndex.current += countNestedBlueprints(entry.blueprint_book.blueprints)
                rows.push(
                    createStaticRow(
            ...
            rows.push(createStaticRow('Unknown entry', x, y))
        }
    }
}
...
export class BookDialog extends Dialog {
    ...
    public constructor(book: Book) {
        ...
        viewport.hitArea = new Rectangle(0, 0, VP_W, VP_H)
        ...
        const onWheel = (e: WheelEvent): void => {
            ...
            this.m_Rows.y = Math.min(
                0,
                Math.max(-maxScroll, this.m_Rows.y - Math.sign(e.deltaY) * ROW_HEIGHT)
            )
