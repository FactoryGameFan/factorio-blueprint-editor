...
- TypeScript has pre-existing type errors in Space Age code (`as any` casts used where prototype types don't match runtime data from data.json). Most of what is left sits in `spriteDataBuilder.ts` (326 of the 513) and is the same problem viewed through `strictNullChecks` - see issue #22 for the plan and the current count
