...
- Logistic chest filters can be written again (issue #64), but only paste-settings reaches the setter: `ChestEditor` is still commented out of `UI/editors/factory.ts` under `// TODO: update using sections`, so there is no dialog for editing a chest's filters or request counts. Re-enabling it is its own job
