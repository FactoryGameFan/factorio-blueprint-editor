...
- `tests/entity-accessors.spec.ts` - Tallies what every `Entity` accessor returns (a value, an empty list, or nothing) across all 367 blueprints in `test-blueprints/`, so a getter that starts returning `[]` where it returned `undefined` shows up. Fixed point, not a refreshable snapshot
