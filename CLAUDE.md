...
- TypeScript has pre-existing type errors in Space Age code (`as any` casts used where prototype types don't match runtime data from data.json). `spriteDataBuilder.ts` is now clear, so the remaining 29 are all UI and container code (globals 5, Book 4, TileContainer 4, InventoryDialog 3, util 3, then a long tail); use `need(e, 'field')` in the sprite builder rather than reading an optional prototype field directly - see issue #22
