...
- TypeScript has pre-existing type errors in Space Age code (`as any` casts used where prototype types don't match runtime data from data.json). `spriteDataBuilder.ts` is now clear, so the remaining 61 are all UI and container code (actions 9, Filters 6, Modules 5, globals 5, Book 4, TileContainer 4, then a long tail); use `need(e, 'field')` in the sprite builder rather than reading an optional prototype field directly - see issue #22
