...
3. `packages/editor/src/core/factorioData.ts` loads `data.json` at runtime as `FD` (FD.entities, FD.items, FD.fluids, FD.signals, FD.recipes, FD.tiles). It also holds the shape-tolerant readers for data that does not match its declared type - `recipeIngredients`/`recipeResults`/`localisedName`, see Space Age Specifics
...
**Restart the dev servers after editing a module that initializes once.** `factorioData.ts` is the one that bites: `FD` is filled in by `loadData` at startup, and Vite's HMR re-evaluates the module on edit without re-running it, so `FD` goes back to `{}` while the app keeps going. Playwright then fails somewhere that has nothing to do with the change - the observed shape was `TypeError: Cannot read properties of undefined (reading 'requester-chest')` inside an AJV custom keyword, in a spec that had not been touched. It reads exactly like a regression. One `npm run localpreview` restart and the same suite went from 1 failed to 97 passed.
...
- Buffer and requester chests show **30** filter slots, which is `Entity.filterSlots`' name-based fallback rather than a real prototype limit - neither declares `max_logistic_slots`, and 2.0 logistic sections have no fixed count. The `// TODO: find a way to fix this properly` on that branch is still open
