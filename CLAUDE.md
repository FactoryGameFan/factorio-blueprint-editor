...
- `IFilter` is index/name/count, so the editor does not model a filter's **quality** or **comparator** even though every filter in the corpus carries both. A write preserves them on a slot that still holds the same item and cannot carry them anywhere else - pasting settings between chests drops them to the Factorio defaults
