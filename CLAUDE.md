...
- `tests/splitter-wires.spec.ts` - That a splitter can carry circuit wires at all, which nothing had answered until the corpus went public turned up a real one (issue #186): `getCircuitConnector` and `getMaxWireDistance` were both missing a `case 'splitter'`, and the two failed nothing alike, which is why one synthetic spec covers both rather than leaving it to the corpus. The connector half is covered loudly by accident - nothing catches around `WiresContainer.add`, so a splitter with no connection point threw and took the _whole_ blueprint down with it, and the overlay, sprite-generation and sprite-data specs all fail alongside it because a blueprint that will not load produces no digests to tally. The distance half was covered by nothing at all: falling to `default: return 0` only makes `WiresContainer.createWire` draw the wire at alpha 0.3, as if the ends were out of range, which no spec reads, and `entity-accessors.spec.ts` buckets `maxWireDistance` into value/empty/nothing/threw where 0 and 9 both land in `value`. Mutation-checked: deleting the distance case alone leaves Playwright, vitest and `vp check` all green. Synthetic on purpose rather than named against the real blueprint that found it, since #186 is itself the demonstration that which corpus file happens to hold a wired splitter is not a stable thing to assert
