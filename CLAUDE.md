...
- `packages/editor/src/core/bpString.ts` - Blueprint string decode/encode, AJV schema validation. Validation is lenient - warns on unknown items/signals, and `stripUnknownPrototypes` drops entities _and_ tiles that FD does not have, reporting the two kinds separately. Tiles were added in issue #46: they were left in, and an unknown one then threw in `TileContainer` on `FD.tiles[name].variants`, taking the whole load down while an unknown entity merely warned. It exports three error classes and the newest one, `EmptyBlueprintStringError`, exists to be **recognised** rather than to carry information: `createBPImportError` renders anything it does not recognise through `createErrorMessage`, whose text ends "report this bug on github", so an empty `?source=` or a paste from an empty clipboard asked the user to file a bug (issue #298). It **rejects** rather than resolving an empty `Blueprint`, which is what that issue first proposed - every caller pipes a resolved value into `loadBp` -> `Editor.loadBlueprint`, which assigns `G.bp`, and `History` is per-`Blueprint`, so resolving would have turned a confusing message into a silent wipe on any stray Ctrl+V. The guard sits **after** the whitespace strip so `'   '` reaches it too.
...
- `packages/editor/src/containers/PaintBlueprintContainer.ts` and `PaintBlueprintEntityContainer.ts` - Pasting a copied selection. `placeEntityContainer` **plans every entity and then places every entity, in two separate loops**, and that split is the whole of issue #163 rather than a style choice: planning inside the placement loop asks a grid the previous placement has already mutated, so from the second entity on a legal one can be silently dropped - measured with two `curved-rail-a` two tiles apart, a pair the game accepts and whose 2x6 editor rectangles overlap. Collapsing it to `container.placeEntityContainer(container.planPlacement())` type-checks and reads as a pure simplification, which is why the reason is written at that site; making `placement` a required parameter stops a caller _forgetting_ to plan and cannot stop one planning in the wrong place. The preview agrees with the result because `checkBuildable` **asks** `planPlacement` rather than repeating its three grid questions (issue #181) - nothing in `tests/` reads a sprite tint, so a second hand-written copy of that expression would be invisible to the entire suite, and the agreement has to be structural. `planPlacement`'s two reusing arms deliberately re-derive their targets during execution instead of carrying them in the plan: an earlier planned action can destroy the object planning found, while a second lookup can only answer the live entity or safely nothing.
...
- `packages/editor/src/core/zoomLevels.ts` - The zoom ladder, measured off the game rather than chosen (#206, #211). Pure and FD-free, so its tests run under `vp test` in **CI**, which mattered more when it was written than it does now: nothing under `tests/` read a zoom level before it, and Playwright ran nowhere but a laptop (#210). The browser suite gates PRs too as of the `e2e` job, so the split is now about **cost** rather than coverage - `vp test` answers in seconds inside `checks`, the browser job takes **three to four** minutes across four sharded runners - its slowest shard measured 213s, 225s and 250s over three runs, down from 774s in one job. Runner throughput varies enough that a single sample is not a figure worth quoting. The step is `2^(1/7)` - seven notches to a doubling, ladder anchored at exactly 1.0 - and `ZOOM_LEVELS` is **computed** from that closed form rather than committed as a literal, which is the opposite of `railSignalSpots.ts` and for the stated reason: that table is 152 placements with no formula, this one is a formula, and a literal would be a transcription that can drift where no reviewer sees it. Three things here are measured and easy to get backwards. `stepZoom` snaps to the **nearest** rung and then moves one index - from 0.83 a notch in gives 0.905724 under both that rule and "next rung in the direction of travel", so only a notch **out** separates them, and the game answers 0.742997. A step that would overshoot a limit lands on the limit **exactly**, so `ZOOM_MIN`/`ZOOM_MAX` are reachable and are not themselves rungs - which is why coming off the ceiling skips one, 3.0 to 2.6918. And `ZOOM_MIN` is **0.1, the game's map editor floor, not its world-view floor**: the latter is a rule, at most 200 tiles across, and 30 of the 367 corpus blueprints exceed it. The `=` and `-` keybinds are what step it (`Editor.zoomStep`), added because the wheel does **not**: `WheelZoom` moves a continuous position measured in rungs, held as a number and added to rather than recomputed from the scale, because that is what keeps zoom exactly reversible; it clamps the **position**, not just the scale, or scrolling hard against a limit banks travel that must be unwound before the view moves again
...
supports after the rails they hold - measured against the old, gitignored
`wormeyman-tests/` corpus (issue #186 replaced it with the committed
`test-blueprints/`, and the figure cannot be re-measured against that corpus
without redoing the walk): radius 11 refused 1005 of 5922 rails, and radius
_1000_ still refused 38, so no radius fixes an order dependency (#150); and the
reachable failure was nearly empty, 0 of 5922 rails lacking a support. Two more
method notes, both in the probe README. **A probe entity's own
...
And what zoom the game uses (issues #206 and #211,
`tools/oracle/fixtures/zoom-limits.json`), the first probe here that **needs a
human**. Zoom belongs to a `LuaPlayer`, so `--create` cannot reach it - a created
map has nobody in it - and `probe-zoom-limits.mjs` therefore runs the graphics
client and does **not** `error("DUMPED-OK")` out: the automated half writes its
dump and the mod keeps sampling while a person scrolls, appending as it goes so a
quit loses nothing. The step is `2^(1/7)`, seven notches to a doubling, anchored
at exactly 1.0, with all 23 non-clamp values exact to 4.2e-10 in `n`; a notch
from between two rungs snaps to the **nearest** and then steps; a step that would
overshoot a limit lands on the limit exactly; the ceiling is 3 and the floor is a
**rule**, at most 200 tiles across the window. The units claim the design rested
on is confirmed to 1e-9 and only at 32 px per tile against the window in
**logical** pixels.

Four method notes came out of it, all in the probe README, and the first two are
the transferable ones:

- **"No headless probe can reach it" is not the same claim as "not worth
  measuring."** #211 wrote the per-notch step off as unreachable and the design
  proposed shipping a guessed 1.33 in its place. The real answer is three real
  notches away from that guess, and getting it took one sitting.
- **Which hypotheses a probe value can separate is part of the question.** Three
  rules for an off-rung notch agree on every value that is already a rung. 0.83
  sits below its gap's midpoint, so a notch **in** cannot tell nearest-then-step
  from directional - both say 0.905724 exactly - and only a notch **out**
  separates them. Reading the first probe as a verdict would have adopted the
  wrong rule with a number matching to nine digits.
- A **script-set value is not a measured one** and enters the same sample stream;
  tag what the probe itself wrote and exclude it by tag, not by value.
- **Ask what a limit _is_ before comparing it to a number.** `zoom_limits` has
  three fields and they are not the same kind of thing - `furthest` is a rule
  rather than a zoom, so comparing it to a readback answers DISAGREE for a limit
  that is perfectly correct.

This is also the third measurement here to **refuse the change it was capturing
evidence for**: #206's stated goal is the game's range, but 30 of the 367 corpus
blueprints are wider than the game's 200-tile world-view floor - the widest is
397, needing 0.151 where that floor is 0.300 - so adopting it would make 8% of
real blueprints impossible to view whole. The game's **map editor** floor, 0.1,
is the number that transfers. And the binary settled the follow-up question when
"scrolling feels chunky" came back from driving it: there is no `smooth` or
`interpolate` symbol anywhere near `CameraBase`, no smooth-zoom setting in the
locale, and no stored `2^(1/7)` constant in all 208 MB, so the engine snaps and
its smoothness is **cadence** - measured at one rung per tick, against the 100 px
per rung a threshold would demand.
...
Automated tests that load blueprint `.txt` files from `test-blueprints/` against the running dev server, capture console warnings/errors, and generate diagnostic reports.
...
- `tests/zoom-ladder.spec.ts` - That a wheel event over the canvas actually reaches the zoom and moves the viewport scale (#206). Deliberately thin, because the arithmetic is unit tested in `packages/editor/src/core/zoomLevels.test.ts`, which answers in seconds inside `checks` - duplicating the ladder here would pay the five-minute browser job for something the cheap gate already covers. Written when Playwright did not run in CI at all, where the argument was coverage rather than cost. Three things it was shaped by, each of which cost a run. **A wheel read races the wheel event**: `page.mouse.wheel` resolves once the event is dispatched, not once the page has handled it, so an immediately following `page.evaluate` reads one event behind - measured, five notches read back as four and a later read then jumped two rungs catching up, which looks exactly like dropped events. A timeout or a fuzzy "about one rung" assertion would have buried it; the spec syncs on a frame instead, and three consecutive runs then gave identical numbers. **The fitted scale for a small blueprint is the ceiling** - two chests ten tiles apart fit at exactly 3, where zooming in is a no-op and half the assertions are vacuous, so the fixture is 100 tiles wide and every test starts off-rung. And the wheel is **continuous**, so nothing here asserts landing on a rung; `BlueprintContainer.zoom()` still steps the ladder and is unit tested rather than driven, since no keybind reaches it yet
- `tests/zoom-keybinds.spec.ts` - The `=` and `-` keybinds, and the only place the **stepped** zoom is driven by real input. `zoom-ladder.spec.ts` cannot cover it and never will: the wheel is continuous and deliberately never lands on a rung, so nothing else exercises `stepZoom` through the app - the ladder became test-only the moment the wheel stopped quantising, which is what these binds exist to fix. The game has the same two controls. Bound on `e.code` rather than on the character, so they work on layouts where `=` needs a modifier. Note what the last test is doing and why the obvious version of it is wrong: `exportKeybinds` writes only the binds that **differ** from their default, so a fresh profile's `keybinds2` is empty and asserting the action names appear in it fails for a reason that has nothing to do with the code. It seeds `{zoomIn: 'KeyJ'}` with `addInitScript` instead and checks that `=` then does nothing while `J` steps - which proves the action is in the registry under that name, that a stored bind reaches it at startup, and that zoom runs through the action system rather than a listener hardcoded to a key the settings pane could never show
...
- `tests/entity-accessors.spec.ts` - What every `Entity` accessor answers across all 367 blueprints, in two records. The eleven open-set accessors are tallied by **shape** (a value, an empty list, nothing, or threw), which is what catches a getter returning `[]` where it returned `undefined`. The fourteen with a small closed value set are recorded as **exact histograms** (issue #189), because the shape tally cannot see a value: `0` and `9` both bucket as `value`, and eight accessors are total functions whose four-bucket tally was pinned at exactly `entityCount` and so could not move at all. That is the structural reason #186's splitter bug survived the corpus. Four assertions run before the fixture comparison so a structural problem is named rather than arriving as a diff. Two are membership guards over three independent artifacts - the list, the tally and the fixture: `TALLIED_VALUES` must declare the same keys as the fixture's `values`, which catches an accessor added to or removed from the list without the fixture following, and every accessor in `TALLIED_VALUES` must actually have been recorded, which catches one that stays declared but gets skipped by the collection loop. Neither shadows the other - removing a name from the list drops it from the collection loop's own input too, so both sides of the second check lose it in lockstep and only the first fires. The other two are that every histogram must cover every entity, and that none may exceed 16 distinct keys, so an accessor that turns open-set says so instead of dumping a thousand-line diff. Note the `shape` half has no equivalent membership guard; that asymmetry is deliberate and out of scope for #189. Fixed point, not a refreshable snapshot: the sanctioned way to move a histogram is to prove it folds back to the previous four buckets, which is how the #189 recapture was justified
...
- `tests/blueprint-sources.spec.ts` - The `?source=` handlers in `bpString.ts`, which had no coverage at all until issue #124: nothing under `tests/` mentioned `/corsproxy` or any of the nine hostnames, so every arm could have been deleted or given the wrong URL with the suite green. The corpus cannot reach them **by construction** - every file in `test-blueprints/` starts with `0`, takes the `DATA[0] === '0'` branch and never builds a URL - and neither can a unit test, since the function ends in `decode`, which needs FD. What makes it cheap is that every arm fetches through `/corsproxy?url=<encoded>`, so `page.route` reads the rewritten URL off the intercepted request and `route.fulfill` supplies the body. Each case asserts **two** things, and the second is the point: the URL catches a typo in a template literal, and loading the result catches a change to the _parse_ step - `gist` reads `data.files[<first key>].content` and `factorio.school` reads `data.blueprintString.blueprintString`, and a rename at either end leaves the URL perfectly correct and the blueprint empty. Mutation-checked three ways, each failing exactly one test. Note the factorio.school arm is covered twice, since a URL already pointing at `/api/` is passed through untouched and read as text rather than JSON. The last test pins a **non**-happy path that is not an endorsement: `fetchData` rejects on `!response.ok` and nothing else, so a host answering **200 with an HTML login or error page** hands markup to `decode()` and the user sees a corrupt-blueprint error naming nothing about the URL - measured on Dropbox while probing #98
- `tests/empty-source.spec.ts` - What the editor says when there is nothing to import (issue #298), and the only place the two live routes to it are driven: an empty `?source=` at startup and a Ctrl+V from an empty clipboard. The arithmetic is unit tested in `packages/editor/src/core/bpString.test.ts`, which is **FD-free on purpose** - the guard has to run before anything that reads blueprint data, so a test needing `loadData` first could not tell a working guard from one sitting below the first FD read. Two things here are not about the message. The paste test asserts the **blueprint**, not the toast, because issue #298's own proposal - resolve an empty source to an empty `Blueprint` - spells the toast identically and silently clears the user's work; its `loadedEntityCount` returns 0 for `encodeCurrent`'s `undefined` so that mutation reads "expected 1, received 0" rather than dying in a helper. And the second test looks like a control until you try `throw` instead of `return Promise.reject`: startup hangs `.catch` off the **result** of `getBlueprintOrBookFromSource`, so a synchronous throw goes straight past it and the editor never starts, while the paste route reaches the same function through `.then` and cannot tell the difference. Five mutations in the spec header, measured
- `tests/blueprint-round-trip.spec.ts` - Pins the decode -> model -> serialize path over all 367 blueprints: counts, a checksum over the in-model entity/tile positions, a checksum over the serialized positions, and a hash of the serialized output. The two checksums are not redundant - `serialize()` re-centres through `getCenter()`, so the serialized one is blind to a `getOffset` change and only the model one catches it. Fixed point
...
- `tests/splitter-wires.spec.ts` - That a splitter can carry circuit wires at all, which nothing had answered until the corpus went public turned up a real one (issue #186): `getCircuitConnector` and `getMaxWireDistance` were both missing a `case 'splitter'`, and the two failed nothing alike, which is why one synthetic spec covers both rather than leaving it to the corpus. The connector half is covered loudly by accident - nothing catches around `WiresContainer.add`, so a splitter with no connection point threw and took the _whole_ blueprint down with it, and the overlay, sprite-generation and sprite-data specs all fail alongside it because a blueprint that will not load produces no digests to tally. The distance half was covered by nothing at all _when this spec was written_: falling to `default: return 0` only makes `WiresContainer.createWire` draw the wire at alpha 0.3, as if the ends were out of range, which no spec reads, and `entity-accessors.spec.ts` bucketed `maxWireDistance` into value/empty/nothing/threw where 0 and 9 both landed in `value`, so deleting the distance case alone left Playwright, vitest and `vp check` all green. **That is no longer true, and it was closed twice from opposite directions** - #205's `wire-switch-completeness.test.ts` catches the deletion from the prototype side under `vp test`, which is the half that runs in CI, and #189 moved `maxWireDistance` into this spec's exact-histogram half, where the same deletion moves 4202 splitters from the `9` key to the `0` key. Measured both ways, not reasoned. The lesson to carry is the one that made the claim go stale silently: **a mutation-check claim in a header is a measurement with a date on it**, and the thing that invalidates it is usually a change to a _different_ file that never opens this one. Synthetic on purpose rather than named against the real blueprint that found it, since #186 is itself the demonstration that which corpus file happens to hold a wired splitter is not a stable thing to assert
...
**Test blueprints** are organized by collection in `test-blueprints/` (EARN, JEPAKAZOL), committed and public as of issue #186. Each `.txt` file contains a raw Factorio blueprint string. `test-blueprints/README.md` carries the attribution, the provenance and what the set is chosen for.

**Every blueprint in the corpus declares 2.0.32 to 2.1.12** - 12 files, 402 versioned blueprints and books (367 blueprints, 35 books, counted the way `blueprint-round-trip.spec.ts` flattens a book into its contents rather than itself). That excludes upgrade and deconstruction planners, which also carry a version: a decode of every top-level object in the corpus counts 418 versioned things, the other 16 being those 16 planners, themselves all 2.0.45 to 2.0.73, so the "all post-2.0" claim below holds across the full 418 and not only the 402. So the corpus still cannot exercise any version-conditional branch: the pre-2.0 arms of `Blueprint.ts`'s guards and all of `nameMigrations.ts` are invisible to it, and code that is wrong for old blueprints passes the entire suite. That is what hid issue #40. Version-conditional code needs a synthetic blueprint at a chosen version - use `tests/helpers/encode-blueprint.ts`. Three other things the corpus cannot reach: a name FD does not have (every file loads clean), anything behind pointer or keyboard input, and **any pre-2.0 data shape** - no file carries `control_behavior.filters` or a flat `request_filters` array, so the shape migrations in `Blueprint.ts` are invisible to it too, whether or not they are version-gated (`tests/pre-2-0-shape-migrations.spec.ts`).
...
**Do not edit anything under `packages/` while a Playwright run is in flight.** Vite hot-reloads the page under the running spec, which dies on `page.evaluate: Execution context was destroyed, most likely because of a navigation` - in a spec that has nothing to do with what was edited, so it reads as a genuine flake in someone else's code. Measured while mutation-checking `zoomLevels.ts` during a full run: `chest-filters` failed that way, and a clean re-run with no edits was 187 passed. Mutation checks and a suite run are both cheap; doing them at the same time is what is expensive.
...
**This suite runs in CI**, as the `e2e` job in `.github/workflows/ci.yml`, and did not until then - the note used to be that it ran nowhere but a maintainer's laptop (#210). The gap was not theoretical: PR #222 arrived with every check green and a `blueprint-round-trip.spec.ts` failure in it, dropping `position-relative-to-grid` from every absolutely-snapped blueprint - 325 of the corpus's 367. Nothing `vp check` or `vp test` can see, because the decode -> model -> serialize path needs `FD` loaded, which needs a browser and the sprite server. Three things about the job worth knowing. It reuses `npm run localpreview` rather than carrying its own copy of how to start the two servers, so the port checks and the tied lifetimes are the same ones locally - **a second copy would be a bound written down twice**. The browser cache is keyed on the **resolved** `@playwright/test` version read out of `node_modules`, not on the caret range in `package.json`, because the resolved version is what decides the browser build; keying on the range serves yesterday's browser to today's Playwright and fails as a missing `chrome-headless-shell`. And `deploy` waits on **both** `checks` and `e2e`, so a change that type-checks and unit-tests clean while breaking the editor in a browser cannot reach fbe.factorygamefan.com - #222 is the worked example, and `checks` was green on it. That costs the deploy path **three to four minutes**, the slowest of the four shards (213s, 225s and 250s over three runs); it was thirteen before the suite was sharded, which is most of why it was. Note this sentence has carried a wrong number twice - "five minutes" guessed from a laptop, then thirteen after #224 corrected the others and missed this one - so if it disagrees with `ci.yml`, believe `ci.yml` and re-measure. Note `serve` became a declared devDependency in the same change: `localpreview` used to fetch it with `npx --yes`, which was fine on a laptop and is not fine as the one run-time-resolved package in a pipeline whose whole point is `vp install --frozen-lockfile`.
